...
Module MP
    ...
    Sub Main(pno As String)
        ...
        con.Open()
        cmd.CommandText = "select Constituency,MP from VoterDB where VPhno='" + pno + "'"
        cmd.Connection = con
        reader = cmd.ExecuteReader()
        While reader.Read()
            cons = reader("Constituency")
            voterstatus = reader("MP")
            ' MsgBox(cons)
        End While
        reader.Close()
        Dim SAPI
...
line1:
            i = 0
            cmd.CommandText = "select * from ContestantDB where Constituency ='" + cons + "' and Post='MP'"
            cmd.Connection = con
            reader = cmd.ExecuteReader()
            While reader.Read()
            ...
            cmd.ExecuteNonQuery()
            cmd.CommandText = "update VoterDB set MP='Yes' where VPhno='" + pno + "'"
            cmd.ExecuteNonQuery()
